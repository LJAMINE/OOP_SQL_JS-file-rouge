...
function getFirstEmployeeFirstName(employ) {
  return employ[0].firstName;
}
...
function getLastEmployeeLastName(employ) {
  return employ[employ.length - 1].lastName;
}
...
function getEmployeeDepartmentById(array, id) {
...
function hasInactiveEmployees(array) {
  for (let i = 0; i < array.length; i++) {
...
function getHighestPaidEmployee(array) {
  let max = array[0].salary;
  for (let i = 0; i < array.length; i++) {
    if (max < array[i].salary) {
      max = array[i].salary;
      person = array[i];
    }
  }
...
function countEmployeesInDepartment(array, depar) {
  let count = 0;
  for (let i = 0; i < array.length; i++) {
